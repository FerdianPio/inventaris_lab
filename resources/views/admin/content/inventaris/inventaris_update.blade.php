...
<div class="row">
    <div class="col-12">
        ...
        @endif
        <form action="{{url('inventaris/update/'.$inventaris->id_inv)}}" method="post" style="padding-top: 10px">
            @csrf
            <div class="form-group">
                <label for="kodeInventaris" class="form-label">ID Inventaris</label>
                <input type="text" name="kodeInventaris" id="kodeInventaris" value="{{$inventaris->id_inv}}" class="form-control" disabled>
            </div>
            <div class="form-group">
...
{{-- @push('footer')
    <script>
        $("#inv_barang_id").val('{{ $inventaris->inv_barang_id }}');
        $("#inv_user_id").val('{{ $inventaris->inv_user_id }}');
        $("#inv_status").val('{{ $inventaris->inv_status }}');
    </script>
@endpush --}}
